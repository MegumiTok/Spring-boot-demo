...
class Pra1ServiceTests {
	...
	void testConvertObjectToJson() throws IOException {
		 ...
		 * 上記みたく明示的にインスタンスを生成した場合、それはSpringのDIとは関係なく、手動で生成されたオブジェクトが利用される。 Spring
		 * Bootのテストコンテキスト（@SpringBootTest が付与されたクラス）では、@Autowired を使用してBeanを注入することが推奨。
		 * これにより、Springが管理するBeanが適切にDIされ、テスト時にもSpringのコンテキストが活用される。
		 * 一部の特定のケースやコード部分では、手動でオブジェクトを生成することが適切な場合もあるが、Spring boot
		 * を使っているなら恩恵に授かった方がいいってことだと思う。
